...
def astar(puzzle, goal): 
    ...
    openset = [set() for i in range(50)]
    openset[0].add((h(puzzle, goal), puzzle, 0, puzzle.index("_")))
    closedset = {}; 
    while(True): 
        ...
        if(puz[1] == goal): 
            return findpath(puzzle, goal, closedset, puz, goaldict) 
        for nbr in neighbors(puz[1], puz[3], goaldict): 
            if (f:=(hfast(nbr, puz, puz[0] - puz[2], goaldict, goal) + puz[2] + 1)) < smallestf: smallestf = f
...
def hfast(puzzle, lastpuz, oldh, goaldic, goal):
      val = oldh +puzzle[2] 
      return oldh +puzzle[2] 
      
def findpath(puzzle, goal, seen, beforegoal, goaldict):
    plist = [goal] 
    if(goal == puzzle): return plist
    maxlevel = beforegoal[2]; val = (beforegoal[1], beforegoal[3]) 
    while(not maxlevel == 0): 
        for nbr in neighbors(val[0], val[1], goaldict ): 
...
def neighbors(puzzle, indx, goaldic): 
    parseMe = [] 
    if((indx)) % Gwidth != (Gwidth - 1): parseMe.append((puzzle[:indx] + puzzle[indx + 1] + puzzle[indx] + puzzle[indx + 2:], indx + 1, -1 if (indx + 1) % Gwidth  > goaldic[puzzle[indx + 1]] % Gwidth else 1))
    if(indx % Gwidth != 0): parseMe.append((puzzle[:indx-1] + puzzle[indx] + puzzle[indx-1] + puzzle[indx + 1:], indx - 1, -1 if (indx - 1) % Gwidth  < goaldic[puzzle[indx - 1]] % Gwidth else 1))
    if(indx + Gwidth < len(puzzle)):  parseMe.append((puzzle[:indx] + puzzle[(gwidthplusindx:=indx + Gwidth)] + puzzle[indx + 1:gwidthplusindx] + puzzle[indx] + puzzle[gwidthplusindx + 1:], indx + Gwidth, -1 if  (indx + Gwidth)//Gwidth > goaldic[puzzle[indx + Gwidth]]//Gwidth else 1))
    if(indx - Gwidth >= 0 ):  parseMe.append((puzzle[:(indxminusGwidth:=indx - Gwidth)] + puzzle[indx] + puzzle[indxminusGwidth + 1: indx] + puzzle[indxminusGwidth] + puzzle[indx + 1:], indx - Gwidth, -1 if  (indx - Gwidth)//Gwidth < goaldic[puzzle[indx - Gwidth]]//Gwidth else 1))
    return parseMe
def main(): 
    goal = ""
    goalset = False
