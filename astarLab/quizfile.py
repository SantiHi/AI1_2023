...
def solved(puzzle): 
    for indx, v in enumerate(puzzle):
        if v == "_": continue
        if(indx % Gwidth != (Gwidth - 1)): 
            if not puzzle[indx + 1] == "_":
                if( abs(int(puzzle[indx]) - int(puzzle[indx + 1]) == 1)): return False 
        if(indx % Gwidth != 0):
            if not puzzle[indx - 1] == "_": 
              if( abs(int(puzzle[indx]) - int(puzzle[indx -1]) == 1)): return False
        if(indx + Gwidth < len(puzzle)):
            if not puzzle[indx + Gwidth] == "_": 
                if( abs(int(puzzle[indx]) - int(puzzle[indx + Gwidth]) == 1)): return False 
        if(indx - Gwidth >= 0 ):
            if not puzzle[indx -Gwidth] == "_": 
             if( abs(int(puzzle[indx]) - int(puzzle[indx - Gwidth]) == 1)): return False
    return True

def solved2(root, puzzle): 
    for indx, v in enumerate(puzzle):
        if(v == root[indx]): return False 
    return True
    ...
    if puzzle == goal: 
        ...
        if(solveable(puzzle, goal)):
            while(len(parseT) > 0): 
                ...
                for i in neighbors(temp[0]): 
                    if solved2(puzzle, i):
                        sol.append((i, temp[1] + 1))
                    if(not i in seen): 
                        seen.update({i:temp})
                        parseT.insert(0, (i, temp[1] + 1))
        print(sol.pop())
        print_puzzle([puzzle])
        return []; 
