...
def bruteForce(pzl, indx): 
    ...
    l = [(len({pzl[nbr]for nbr in neighbors[indx] if pzl[nbr] != "."}), indx) for indx, val in enumerate(pzl) if val == "."] 
    i = max(l)[1]; STATSs["OptimalCC"] += 1
    for val in symbolset:     
        subPzl = pzl[:i] + val + pzl[i + 1:]
