...
def setGlobals(pzl): 
    global constraints; global constraintsthatmakesense; global neighbors; global symbolset
#constraints col 
...
def bruteForce(pzl, choicelist, indx): 
    if isInvalid(pzl, indx): return "" 
    if isSolved(pzl): return pzl 
    l = choicelist[::]
    choice, l = findOptimalChoices(pzl, l)
    for val in range(1, 10):     
        subPzl = pzl[:choice] + symbolset[val] + pzl[choice + 1:]
        bF = bruteForce(subPzl, l, choice)
        if bF: return bF 
    

def findOptimalChoices(pzl, l): 
    indx = l.pop(p:=max(l))[1]; 
    for i, tup in enumerate(l): 
        if indx in neighbors[tup[1]]: 
            l[i] = (tup[0] + 1, tup[1])
    return (indx, l)
def findspaces(pzl): 
    lst = [] 
...
def main(): 
    ...
    for line in f1: 
        ...
        if(not isInvalidonce(puz)): 
            solved = bruteForce(puz, {(sum([1 for nbr in neighbors[indx] if puz[nbr] != "."]), indx) for indx, val in enumerate(puz) if val == "."},  puz.index("."))
        else: solved = "" 
        if solved: 
