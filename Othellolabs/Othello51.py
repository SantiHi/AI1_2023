...
import re
def makeGlobals(): 
    board = "xoooooo.xxoooooooooooooxooxooox..xxoooxxxxxxxooxxxxxxxoooxxxxxxo" 
    moves = []
    currenttoken = "o"
    if(args): 
        for val in args: 
...
def negamax(brd, tkn):
  ...
  if brd.count(".") == 0 or (not (fm:=findMoves(brd, tkn)) and not (efm:=findMoves(brd, eTkn))): return [brd.count(tkn)-brd.count(eTkn)]
  bestSoFar = [-65]
  possibMove = findMoves(brd, tkn)
  if not possibMove:
    oppPosMove = findMoves(brd, eTkn)
    if not oppPosMove:
      return [brd.count(tkn) - brd.count(eTkn)]
    nm = negamax(brd, eTkn)
    return [-nm[0]] + nm[1:] + [-1]
  for mv in possibMove:
